...
describe("Token Multisender", function () {
  before(async function () {
    ...
    const ThrustpadTokenFactory = await ethers.getContractFactory(
      "ThrustpadTokenFactory"
    );
  ...
  describe("Multisender ↗️↗️↗️", function () {
    it("Should deploy multisender and distribute 1 token each to 100 wallets", async function () {
      ...
      const tsalt = await this.tokenFactory.getdeployedTokensLen(
        this.deployerAddress
      );
      const tokenAddress = await this.tokenFactory.getAddressCreate2(
        tbyteCode,
        tsalt
      );

      await this.tokenFactory.newToken(
        name,
        symbol,
        decimals,
        tokenSupply,
      ...
      const multisenderAddress =
        await this.multisenderFactory.getAddressCreate2(mbyteCode, msalt);
      ...
      const deployedMultisenders =
        await this.multisenderFactory.getdeployedMultisenders(
          this.deployerAddress
        );
    ...
    it("Should deploy multisender and distribute 0.01 ETH each to 100 wallets", async function () {
      ...
      assert.equal(
        await ethers.provider.getBalance(
          "0x6836578dae5d158466813b59598ea1fffff80b1a"
        ),
        ethers.parseEther("1")
      );
    });
